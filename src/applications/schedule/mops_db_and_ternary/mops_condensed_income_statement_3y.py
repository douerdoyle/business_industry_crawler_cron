...
from models.MOPS               import MOPS_Web, MOPS_CondensedIncomeStatement3Y
...
def mops_condensed_income_statement_3y_func():
    main_name = '簡明綜合損益表(三年)'
    try:
        ...
        for stock_symbol, president_no in stk_symbol_pre_no_dict.items():
            print('「簡明綜合損益表(三年)」執行中，統編：{}、公司代碼：{}'.format(president_no, stock_symbol))
            db_year_list = [x.year for x in MOPS_CondensedIncomeStatement3Y.query.filter(MOPS_CondensedIncomeStatement3Y.president_no==president_no, MOPS_CondensedIncomeStatement3Y.stock_symbol==stock_symbol).order_by(desc(MOPS_CondensedIncomeStatement3Y.year)).all()]
            ...
            for year_index, year in enumerate(year_list):
                ...
                db.session.add(MOPS_CondensedIncomeStatement3Y(**db_dictionary))
            db.session.commit()
        ...
        ggg.send_email()
if __name__ == '__main__':
    mops_condensed_income_statement_3y_func()