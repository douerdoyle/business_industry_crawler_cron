...
from models.MOPS               import MOPS_Web, MOPS_BalanceSheet4S
...
def mops_balance_sheet_4s_func():
    main_name = '簡明資產負債表(四季)'
    try:
        ...
        for stock_symbol, president_no in stk_symbol_pre_no_dict.items():
            for i in range (0, 2):
                ...
                try:
                    ...
                    continue
                data_type_this_year = [x.data_type for x in MOPS_BalanceSheet4S.query.filter(MOPS_BalanceSheet4S.stock_symbol==stock_symbol, MOPS_BalanceSheet4S.year==year).all()]
                for data_type_index, data_type in enumerate(data_type_list):
                    ...
                    if data_type in data_type_this_year:
                        MOPS_BalanceSheet4S.query.filter(MOPS_BalanceSheet4S.stock_symbol==stock_symbol, 
                                                        MOPS_BalanceSheet4S.year==year,
                                                        MOPS_BalanceSheet4S.data_type==data_type).update(db_dictionary)
                    else:
                        db.session.add(MOPS_BalanceSheet4S(**db_dictionary))
                db.session.commit()
    except Exception as e:
